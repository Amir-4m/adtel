    ...
    with TelegramClient(StringSession(adm_ts.session), adm_ts.api_id, adm_ts.api_hash, proxy=TELETHON_PROXY) as client:
        ...
        for campaign_post in campaign_posts:
            ...
            try:
                ...
                logger.info(f"read view for post: {campaign_post.id}")
            except FloodWaitError as e:
                logger.warning(f"read view for post: {campaign_post.id} failed, error: {e}")
            except Exception as e:
                logger.error(f"read view for post: {campaign_post.id} failed, error: {e}")

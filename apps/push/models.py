...
class CampaignPushUser(models.Model):
    ...
    def __base_remain_push_data(self):
        qs = CampaignPublisher.objects.select_related(
            'publisher__sheba'
        ).filter(
            publisher__admins=self.user,
            campaign_id=self.campaign_push.campaign_id,
            publisher_id__in=self.campaign_push.publishers.exclude(
                id__in=self.campaign_push.confirmed_channels().values_list('channels__id', flat=True)
...
class CampaignPush(models.Model):
    created_time = models.DateTimeField(_('created time'), auto_now_add=True)
    updated_time = models.DateTimeField(_('updated time'), auto_now=True)
    status = models.SmallIntegerField(_('status'), blank=True, null=True, editable=False)
    ...
    # def __base_remain_push_data(self, user=None):
    #     qs = CampaignPublisher.objects.select_related(
    #         'publisher__sheba'
    #     ).filter(
    #         campaign_id=self.campaign_id,
    ...
    #     )
    #     if user:
    #         qs = qs.filter(publisher__admins=user)
    #     return qs.distinct()
    #
    # def has_push_data(self):
    #     return self.__base_remain_push_data().exists()
    #
    # def get_push_data(self, user):
    #     data = {}
    #     for publisher in self.__base_remain_push_data(user):
    #         data.setdefault(
    #             publisher.publisher.sheba.sheba_number, []
    #         ).append(
    #             {
